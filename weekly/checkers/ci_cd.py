...
class CIChecker(BaseChecker):
    ...
    def _detect_ci_config(self, project: Project) -> Dict[str, Any]:
        ...
        if (project.path / ".github" / "workflows").exists():
            workflow_files = list(
                (project.path / ".github" / "workflows").glob("*.yml")
            )
            workflow_files += list(
                (project.path / ".github" / "workflows").glob("*.yaml")
            )
    ...
    def _detect_cd_config(self, project: Project) -> Dict[str, Any]:
        ...
        if (project.path / "app.json").exists() or (project.path / "Procfile").exists():
            return {"exists": True, "type": "Heroku", "path": "app.json or Procfile"}

        # Docker
        if (project.path / "Dockerfile").exists() or (
            project.path / "docker-compose.yml"
        ).exists():
            return {
                "exists": True,
                "type": "Docker",
    ...
    def _has_testing_in_ci(self, project: Project, ci_config: Dict[str, Any]) -> bool:
        ...
        if ci_config["type"] == "GitHub Actions":
            for workflow_file in ci_config.get("files", []):
                ...
                if content and any(
                    keyword in content.lower()
                    for keyword in ["test", "pytest", "unittest", "tox"]
                ):
                    return True

        # Check other CI configs
        elif ci_config["type"] in [
            "GitLab CI",
            "Travis CI",
            "CircleCI",
            "Azure Pipelines",
